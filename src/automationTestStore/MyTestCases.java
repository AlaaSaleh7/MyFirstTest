...
public class MyTestCases {
	...
	public void AddItemToCart() throws InterruptedException {
		...
		// define for the web element which is a UL tag
		...
		int totalNumberOfItem = listOfItem.findElements(By.tagName("li")).size();
		// create a random index based on the total number of items that i got in the
		// previous line
		int randomIndexForItems = rand.nextInt(totalNumberOfItem);
		...
		listOfItem.findElements(By.tagName("li")).get(randomIndexForItems).click();
		...
		if (LiItem > 0) {
			...
			Assert.assertEquals(ActualValueForCheckOut, ExpectedValueForCheckOut, "Alaa Hi");
